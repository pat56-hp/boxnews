<div class="entry" data-type="{{ $typeofwidget }}" @if(isset($entry->id)) data-entry-id="{{ $entry->id }}" @endif>
    ...
    <div class="embedarea @if(empty($entry->video)) hide @endif ">
        ...
        <div class="moredetail text">
            ...
            <a href="javascript:;" class="trigger"><span class="down">{{ trans('addpost.mored') }}
            <i class="fa fa-angle-down"></i></span><span class="up">{{ trans('addpost.lessd') }} <i class="fa fa-angle-up"></i></span></a>
        </div>
    </div>
</div>
